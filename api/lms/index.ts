...
export class LmsApi extends Api {
  private query = new LmsQuery(this.db)
  private auth = new AuthMiddleware(this.core)
  ...
  handlers = {
    ...
    getLesson: (req, res) => {
      ...
      const nav = this.query.selectNavLesson(courseSlug, lessonSlug)
      if (!lesson) {
        throw new RouteError(404, "aula não encontrado")
      }
      const index = nav?.findIndex((l) => l.slug === lesson.slug) ?? 0
    ...
    completeLesson: (req, res) => {
      try {
        ...
        if (!writeResult.changes) {
          throw new RouteError(404, "erro ao completar aula")
        }
        const progress = this.query.selectProgress(userId, courseId)
        const uncompletedLessons = progress.filter((item) => !item.completed)
        console.log(uncompletedLessons.length)
        if (progress.length && !uncompletedLessons.length) {
